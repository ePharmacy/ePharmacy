...
class CustomerDao extends BaseDao
{
    ...
    public function get_customer_with_email($email, $offset, $limit, $order = "-id")
    {
        ...
        return $this->query("SELECT * FROM customers WHERE LOWER(email) LIKE CONCAT('%', :email, '%') ORDER BY ${order_column} ${order_direction} LIMIT ${limit} OFFSET ${offset}", ["email" => strtolower($email)]);
    }
